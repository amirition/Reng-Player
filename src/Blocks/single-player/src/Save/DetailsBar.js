...
export default function DetailsBar(props ) {
	...
	// Make a color lighter, for background
	...
	const textColor = function (backgroundColor) {
		...
		let shade = (parseInt(result[1], 16) + parseInt(result[2], 16) + parseInt(result[3], 16)) / 3;
		return shade > 128 ? '#333' : '#f1f1f1';
	}

	const barStyle = {
		background: 'linear-gradient( 45deg, ' +
			pSBC(.4, props.backgroundColor) +
			', ' +
			props.backgroundColor +
			', ' +
			pSBC(.3, props.backgroundColor) +
			')'
	}
